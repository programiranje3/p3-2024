...
def pass_simple_function_as_parameter():
    ...
    #
    # def g(h, *args):
    #     print(h(*args) if args else 'Nothing to print')
    #
    # g(f, *green_day)
    # g(f, )
    ...
    def f(a1, *args):
        return str(a1) + ': ' + ', '.join([str(a) for a in args])

    def g(h, a1, *args):
        print(h(a1, *args))

    g(f, 'Green Day', *green_day)
    # g(f, *green_day)
...
def band_details(f_to_decorate):
    ...
    def wrap(*args, **kwargs):
        ...
        print(', '.join([str(a) for a in args[1:]]) if len(args) > 1 else '')
        print(', '.join([str(k) + ': ' + str(v) for k, v in kwargs.items()]) if kwargs else '')
        return v
